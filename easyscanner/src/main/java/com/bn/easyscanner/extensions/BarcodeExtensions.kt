    ...
    return when (valueType) {
        Barcode.TYPE_CONTACT_INFO -> {
            ContactInfoParcelable(
                addressParcelables = contactInfo?.addresses?.map { it.toParcelableAddress() }
                    .orEmpty(),
        ...
        Barcode.TYPE_PHONE -> PhoneParcelable(
            number = phone?.number.orEmpty(),
            type = phone?.type ?: 0
        )

        Barcode.TYPE_SMS -> SmsParcelable(
            message = sms?.message.orEmpty(),
            phoneNumber = sms?.phoneNumber.orEmpty()
        )

        Barcode.TYPE_URL -> UrlBookmarkParcelable(
            title = url?.title.orEmpty(),
            url = url?.url.orEmpty()
        )
        ...
        Barcode.TYPE_GEO -> GeoPointParcelable(
